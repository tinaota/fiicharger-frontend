<template>
    <div class="scroll">
        <div class="mainctrl">
            ...
            <div class="card-8">
                <div class="header">{{ $t('chargingStation.kioskInfo')}}</div>
                <el-table
                    :data="[kioskInfo]"
                    v-loading="isLoading"
                    class="center">
...
<script>
import { $HTTP_getKioskDetail } from "@/api/api";
...
export default {
    ...
    created() {
        ...
        if (!this.curRouteParam.stationId) {
            let temp = window.sessionStorage.getItem("fiics-kioskInfo") ? JSON.parse(window.sessionStorage.getItem("fiics-kioskInfo")) : null;
    ...
    mounted() {
        ...
        this.fetchKioskDetail();

    },
    beforeDestroy() {
        window.sessionStorage.removeItem("fiics-kioskInfo");
    },
    methods : {
        fetchKioskDetail() {
            const that = this;
            let param = {
                kioskId: that.curRouteParam.kioskId
            };
            this.isLoading = true;
            $HTTP_getKioskDetail(param).then((data) => {
                this.isLoading = false;
                if (!!data.success) {
                    this.kioskInfo = {
                                            kioskId: data.kioskInfo.kioskId,
                                            stationId: data.kioskInfo.stationId,
                                            stationName: data.kioskInfo.stationName,
                                            zipCode: data.kioskInfo.zipCode,
                                            address: data.kioskInfo.address,
                                            loc: {
                                                lng: data.kioskInfo.loc.lon,
                                                lon: data.kioskInfo.loc.lon,
                                                lat: data.kioskInfo.loc.lat
                                            },
                                            serviceStartTime: data.kioskInfo.serviceStartTime,
                                            serviceEndTime: data.kioskInfo.serviceEndTime,
                                            countryCode: data.kioskInfo.countryCode,
                                            phone: data.kioskInfo.phone
                                        };
                    this.summary = {
                                        chargeBoxCount: data.kioskInfo.chargeBoxCount,
                                        acChargeBoxInfo: {
                                            count: data.kioskInfo.acChargeBoxInfo.count,
                                            chargeConnectorTypeList: data.kioskInfo.acChargeBoxInfo.chargeConnectorTypeList.slice()
                                        },
                                        dcChargeBoxInfo: {
                                            count: data.kioskInfo.dcChargeBoxInfo.count,
                                            chargeConnectorTypeList: data.kioskInfo.dcChargeBoxInfo.chargeConnectorTypeList.slice()
                                        },
                                        chargeBoxTotalPower: data.kioskInfo.chargeBoxTotalPower
                                    };
        ...
        handleShowDialog() {
            ...
            this.mapDialog.position = this.kioskInfo.loc;

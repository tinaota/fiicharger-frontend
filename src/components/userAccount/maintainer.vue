    ...
    <div class="maintainer">
        <div class="container">
            ...
            <el-table
                ...
                class="moreCol"
                v-loading="isLoading">
                <el-table-column prop="mtLoginId" :label="$t('userAccount.loginId')" :min-width="2"></el-table-column>
                <el-table-column prop="maintainerName" :label="$t('general.name')" :min-width="2"></el-table-column>
                ...
                <el-table-column :label="$t('general.action')" :width="140">
                    <template slot-scope="scope">
                        ...
                        <el-button class="no-bg delete" @click="deleteEvent(scope.row.maintainerId, scope.row.mtLoginId)"></el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog
            ...
            <div class="vertial formVertical">
                <div class="form-item">
                    ...
                    <el-input v-model="dialog.info.mtLoginId" :disabled="dialog.type===1"></el-input>
                </div>
                <div class="form-item">
                    <div class="label">{{ $t('general.name') }}</div>
                    <el-input v-model="dialog.info.maintainerName"></el-input>
                </div>
                <div class="form-item">
    ...
    methods: {
        ...
        openDialog(type, data) {
            ...
            if (!type) {
                ...
                this.dialog.$Api = $HTTP_addMaintainer;
            } else {
                this.dialog.info = {
                                    mtLoginId: data.mtLoginId,
                ...
                this.dialog.$Api = $HTTP_updateMaintainer;
            }
        ...
        updateData() {
            const that = this;
            let params = {
                            maintainerName: this.dialog.info.maintainerName,
                            countryCode: this.dialog.info.countryCode,
                            phone: this.dialog.info.phone,
                            email: this.dialog.info.email
                        };
            if (this.dialog.type) {
                params.maintainerId = this.dialog.info.maintainerId;
            } else {
                params.mtLoginId = this.dialog.info.mtLoginId;
        ...
        deleteEvent(id, loginId) {
            ...
            this.$confirm(i18n.t('general.deleteItem', { item: loginId }), i18n.t('general.hint'), {
                ...
                $HTTP_deleteMaintainer({maintainerId: id}).then(data => {
                    if (!!data.success) {

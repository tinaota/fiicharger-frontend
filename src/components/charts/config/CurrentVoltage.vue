...
<script>
import { $HTTP_getCurrentVoltageGraphDataByTransactionId } from "@/api/api";
...
export default {
    ...
    mounted() {
        if (this.data.transactionId) {
            this.getCurrentVoltageGraphDataByTransactionId(this.data.transactionId);
        }
    },
    methods: {
        getCurrentVoltageGraphDataByTransactionId(id) {
            $HTTP_getCurrentVoltageGraphDataByTransactionId({ transactionId: id })
                .then((res) => {
                    if (res.length > 0) {
                ...
                .catch((err) => {
                    this.responseData = [];
                    let errorMessage = catchErrors("get currentVoltageGraphData", err);
        ...
        convertChartOptions(data) {
            ...
            let currentImport = data.map((item) => item.y.currentImport);
            let currentOffered = data.map((item) => item.y.currentOffered);
            let voltage = data.map((item) => item.y.voltage);
            let legendData = Object.keys(data[0].y).map((item) => {
                if (item === "currentImport") {
                    return i18n.t(`general.current`);
                } else if (item === "currentOffered") {
                    return i18n.t(`general.maxCurrent`);
                } else {
                    return i18n.t(`general.${item}`);
                }
            });
            ...
            let option = {
                ...
                series: [
                    ...
                    {
                        name: i18n.t("general.maxCurrent"),
                        type: "line",
                        yAxisIndex: 1,
                        symbol: "circle",
                        symbolSize: 7,
